...
class _HomeScreenState extends State<HomeScreen> {
  ...
  final List<Map<String, dynamic>> coreFoodCategories = const [
    ...
    {'icon': Icons.eco_outlined, 'label': '샐러드'}, // Changed from 브런치
    {'icon': Icons.delivery_dining_outlined, 'label': '피자'}, // Changed from 간편식
    {'icon': Icons.cake_outlined, 'label': '카페'},
    {'icon': Icons.room_service_outlined, 'label': '파인다이닝'}, // Changed from 다이닝
  ...
  final List<String> situationalTags = const [
    '#데이트 성공확률 높음', '#조용한 자리', '#혼밥 100% 가능', '#줄서먹는집', '#가성비갑', '#친절함', '#사진보다 맛이 진짜임', '#술과 같이 하기 좋음'
  ];

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final currentCategories = _selectedCategory == MainCategory.food ? coreFoodCategories : coreDrinkCategories;
        ...
        slivers: [
              ...
              child: Padding(
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: const Row(children: [ Icon(Icons.search, color: Colors.grey), SizedBox(width: 8), Text("'진짜' '맛집'을 '검색'하세요", style: TextStyle(color: Colors.grey))]),
                  ),
                ),
              ),
            ),
          ...
          SliverPadding(
            ...
            sliver: SliverGrid(
              gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                crossAxisCount: 5, crossAxisSpacing: 8.0, mainAxisSpacing: 16.0, childAspectRatio: 0.9,
              ),
                ...
                (context, index) {
                  ...
                  return InkWell(
                    ...
                    child: Column(
                      ...
                      children: [
                        Icon(category['icon'] as IconData, size: 32, color: Colors.grey[700]),
                        const SizedBox(height: 8),
                        Text(category['label'] as String, style: const TextStyle(fontSize: 12)),
                      ],
                    ),
                  );
                },
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Text("지금 이런 곳은 어때요?", style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
            ),
          ),
          SliverToBoxAdapter(
            child: SizedBox(
  ...
  Widget _buildMainCategoryToggle(ThemeData theme, String text, MainCategory category) {
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          child: Center(
            child: Text(
              text, 
              style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: isSelected ? theme.primaryColor : Colors.black54),
            ),
          ),
        ),
      ),
